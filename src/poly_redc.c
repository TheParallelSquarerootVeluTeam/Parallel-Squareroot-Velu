...
{
  // Variant of reciprocal_tree where the products are presented as a root node and two subtrees.
  // It assumes the reciprocal of the root has already been computed, and computes the reciprocal
  // tree of the left subtree only.

  if(n0 == 0)
    return;

  const int lenr = DEG1[0];
  
  R0[0] = malloc(sizeof(fp)*lenr);
  ...
  if (DEG0[0] == 2 && lenr == 1)
  {
    reciprocal_tree(R0, A0, lenr-1, H0, DEG0, 1, n0-(n0>>1));
    reciprocal_tree(R0, A0, lenr-1, H0, DEG0, 2, n0>>1);
    return;
  }
  ...
  if(leng < lenr)
  {
    for(i = 0; i < lenr && i < DEG0[0]+1; i++)
      fp_copy(R0[0][i], H0[0][DEG0[0]-i]);
    for(i = DEG0[0]+1; i < lenr; i++)
      fp_set0(R0[0][i]);
    reciprocal(R0[0], A0[0], R0[0], lenr, lenr);
  }
  else
  {
  // When parent's inverse was to a greater/equal modulus, this inverse can be obtained from it
    for(i = 0; i < lenr; i++)
      fp_copy(R0[0][i], H1[0][DEG1[0]-i]);
    poly_mul_low(R0[0], lenr, Rroot, leng, R0[0], lenr);
    fp_copy(A0[0], Aroot);
  }
...
{
  // Variant of multieval_unscaled where the trees are presented as one root node and
  // two subtrees with n1 = n>>1 and n0 = n-n1,


  if(n == 0)
    return;
  
  const int DEGH = DEG0[0] + DEG1[0];
  fp g_mod[DEGH];

  poly_redc(g_mod, g, leng, Hroot, DEGH+1, Rroot, Aroot);
  ...
  multieval_unscaled(REM, g_mod, DEGH, R0, A0, H0, DEG0, 0, n-(n>>1));
  multieval_unscaled(&(REM[n-(n>>1)]), g_mod, DEGH, R1, A1, H1, DEG1, 0, n>>1);
  return;
}
...
void multieval_scaled_split_second_branch(fp REM[], const poly G, const poly H0[], //
			   const int DEG0[], const int DEG1[], const int root, const int n)
{
  // Helper function to multieval_scaled_split
    
  const int parent = (root-1) >> 1;
  const int brother = root - 1 + 2*(root & 1);
  const int uncle = parent - 1 + 2*(parent & 1);
  fp fg[DEG0[brother]+1];
    
  poly_mul_middle(fg, H0[brother], DEG0[brother]+1, G, DEG1[0]+1);
    
  
  if(n == 1)
  {
    fp_copy(REM[0], fg[DEG0[brother]]);
    return;
  }

  multieval_scaled(REM, fg, H0, DEG0, 2*root+1, n-(n>>1));
  multieval_scaled(&(REM[n-(n>>1)]), fg, H0, DEG0, 2*root+2, n>>1);
  return;
}


void multieval_scaled_split_first_branch(fp REM[], const poly G, const poly H0[], //
			   const int DEG0[], const poly H1[], const int DEG1[], const int n)
{
  
  // Helper function to multievale_scaled_split

  fp fg[DEG1[0]+1];
  poly_mul_middle(fg, H1[0], DEG1[0]+1, G, DEG0[0]+DEG1[0]);
    
  if(n == 1)
  {
    fp_copy(REM[0], fg[DEG1[0]]);
    return;
  }

  multieval_scaled_split_second_branch(REM, fg, H0, DEG0, DEG1, 1, n-(n>>1));
  multieval_scaled_split_second_branch(&(REM[n-(n>>1)]), fg, H0, DEG0, DEG1, 2, n>>1);
  return;
}
...
void multieval_scaled_split(fp REM[], const poly G, const poly H0[], //
			   const int DEG0[], const poly H1[], const int DEG1[], const int n)
{
  // Variant of multieval_scaled where the product trees are presented as one root node and
  // two subtrees with n1 = n>>1 and n0 = n-n1.

  if(n == 1)
  {
    fp_copy(REM[0], G[DEG0[0] + DEG1[0] - 1]);
    return;
  }
  else
  {
    multieval_scaled_split_first_branch(REM, G, H0, DEG0, H1, DEG1, n-(n>>1));
    multieval_scaled_split_first_branch(&(REM[n-(n>>1)]), G, H1, DEG1, H0, DEG0, n>>1);
    return;
  }
