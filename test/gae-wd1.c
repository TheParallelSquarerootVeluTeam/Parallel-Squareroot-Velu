...
int main()
{
	...
	for(i = 0; i < NUMBER_OF_DIFFERENT_STRATEGIES; i++){
		assert(ROUNDS[i] >= 0);
		for(j = 0; j < (int)(NUMBER_OF_PRIMES[i]); j++){
			assert(L_STRATEGY[i][j] <= N);
			assert(S[i][j] <= N);
		}
		for(j = 0; j < (int)(N-NUMBER_OF_PRIMES[i]); j++)
			assert(W_STRATEGY[i][j] <= N);
	}
