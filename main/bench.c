...
int main()
{
	...
	ticks cc_median = 0, cc_mean = 0, cc_stdev = 0, cc_meanBB = 0;
	...
	{
		// Average value
		if (cc_sample[i] < 3 * cc_min)
			{
				k++;
				cc_meanBB += cc_sample[i];
			};
	...
	printf("Average square root velu\t[running-time (millions): \t(clock cycles (millions): \033[1;32m%7.03lf\033[0m)\n", 	(1.0 * cc_svl) / 1000000.0 );
	printf("\n");
	...
	printf("\n");
	printf("AverageBB\t[running-time (millions): %7.03lfM + %7.03lfS + %7.03lfa = \033[0;32m%7.03lfM\033[0m]\t(clock cycles (millions): \033[1;32m%7.03lf\033[0m)\n", 
	(1.0 * mul_mean) / 1000000.0, (1.0 * sqr_mean) / 1000000.0, (1.0 * add_mean) / 1000000.0, (1.0 * (mul_mean + sqr_mean)) / 1000000.0, 
	(1.0 * cc_meanBB) / 1000000.0 );
	printf("\n");
	printf("out:%d\n", its-k);
